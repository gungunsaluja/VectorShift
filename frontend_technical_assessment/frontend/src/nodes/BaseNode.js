...
const AutoResizeTextarea = ({ value, onChange, placeholder, rows, minHeight, className, style }) => {
...
export const BaseNode = ({ 
          ...
          <label key={field.name} className="block text-xs text-gray-300">
            ...
            >
              {field.options?.map(option => (
                <option key={option.value} value={option.value} className="bg-vs-dark-800">
                  {option.label}
                </option>
              ))}
            </select>
  ...
  const nodeStyle = dynamicStyle && typeof dynamicStyle === 'function' 
  ...
  return (
    ...
    >
      ...
      <div className="mb-2">
        <span className="text-vs-purple-200 font-semibold text-sm">{title}</span>
      </div>
